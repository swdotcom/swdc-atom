    ...
    async buildSlackWorkspacesNode() {
      ...
      if (workspaces.length) {
        workspaces.forEach(workspace => {
          const domain = workspace.meta ? workspace.meta.domain : 'slack'
          const name = workspace.meta ? `(${workspace.meta.name})` : ''
          workspaceHtml += `<li class="list-node-item">
            <div class="list-node">
              <a href="#" id="${workspace.authId}" class="slack-icon">${domain} ${name}</a>
