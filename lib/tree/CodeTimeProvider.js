...
export default class CodeTimeProvider {
    ...
    getAuthTypeLabelAndClass() {
        ...
        if (authType === 'google') {
            return { label: 'Connected using Google', class: 'google-icon' };
        } else if (authType === 'github') {
            return { label: 'Connected using GitHub', class: 'github-icon' };
        } else if (authType === 'software') {
            return { label: 'Connected using email', class: 'envelope-icon' };
        }
        return { label: 'Connected', class: 'envelope-icon' };
    }
        ...
        if (lastCommitInfo) {
            return `
                <li>
                    ...
                    <label for="${id}">${label}</label>
                    <ul>
                        <li><a href=${link} id="${id}-commitId">${lastCommitInfo.comment}</a></li>
                    </ul>
                </li>`;
        } else {
