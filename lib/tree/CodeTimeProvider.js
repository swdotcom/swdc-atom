...
export default class CodeTimeProvider {
    ...
    async buildCommitTreeNodes() {
        ...
        if (folders && folders.length > 0) {
            ...
            for (let i = 0; i < folders.length; i++) {
                ...
                const openChangeDirNodeHtml = this.buildOpenChangesDirNodeItem(
                    `dir-${i}`,
                    basename,
                    currentChagesSummary.insertions,
                    currentChagesSummary.deletions
                );
                openChangesDirNodes.push(openChangeDirNodeHtml);
            }
            const openChangesNodeHtml = this.buildOpenChangesNodeItem(
                openChangesDirNodes
            );
    ...
    buildOpenChangesDirNodeItem(id, label, insertions, deletions) {
        return `
            <li>
                <input type="checkbox" id="${id}" />
                <label for="${id}">${label}</label>
    ...
    buildOpenChangesNodeItem(openChangesDirNodes) {
        return `
        <ul>
            <li>
                <input type="checkbox" id="open-changes" />
                <label for="open-changes">Open changes</label>
                <ul>
                    ${openChangesDirNodes.join('\n')}
                </ul>
            </li>
        </ul>`;
    }
