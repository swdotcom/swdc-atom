...
export default class CodeTimeProvider {
    ...
    async treeGenerator() {
        ...
        if (name) {
            ...
            $('#email-signup').hide();
            $('#signup-divider').hide();
            $('#logged-in-menu').html(this.buildLoggedInNode()).show();
        } else {
            $('#google-signup').show();
            $('#github-signup').show();
            $('#email-signup').show();
    ...
    buildLoggedInNode() {
      ...
      return `
          <li>
              <input type="checkbox" id="${id}" value="${labelInfo.label}" ${checkedProp}/>
              <label for="${id}">${labelInfo.label}</label>
              <ul>
                  <li><a href="#" id="switch-account-button" class="rocket-icon">Switch account</a></li>
              </ul>
          </li>`
    }
