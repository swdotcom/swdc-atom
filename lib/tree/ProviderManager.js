...
providerManager.toggleCodeTimeProvider = async (state) => {
    ...
    if (dock.isVisible()) {
      // check to see if we need to fetch the session summary today
      // since this is a show request
      if (utilMgr.shouldFetchSessionSummaryData()) {
        await sessionAppMgr.updateSessionSummaryFromServer();
      }
      codetimeProvider.rebuildTree();
    }
};
