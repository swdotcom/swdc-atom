...
export default class CustomDashboardManager {
  ...
  async confirm() {
    ...
    try {
      ...
      atom.notifications.addError("Code Time: Error parsing dates")
      this.close();
      return;
    }
    ...
    if (start > end) {
      atom.notifications.addError("Code Time: Start date must come before end date")
    }

    let online = await utilMgr.serverIsAvailable();

    if (online) {
        utilMgr.launchCodeTimeDashboard({start, end});
    } else {
        atom.notifications.addError("Code Time: could not fetch new custom dashboard");
