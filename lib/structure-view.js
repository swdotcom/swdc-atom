...
export default class StructureView {
      ...
      methods: {
        onToggleTreeNode(evt) {
          if (this.CONFIG_DBLCLICK_TO_FOLD_TREE) {
            utilMgr.selectTreeNode($(evt.target), this, {
              toggle: true
            });
          }
        },
  ...
  renderTree() {
    ...
    let html = this.treeGenerator(data2);
        ...
        if (nRow !== e.oldScreenPosition.row) {
          ...
          if (tag && tag.id !== self.treeNodeId) {
            ...
            if ($tag.length > 0) {
              ...
              utilMgr.selectTreeNode($tag, this);
              let ret = utilMgr.getScrollDistance($tag, $(this.element));
