    ...
    return utilMgr
        ...
        .then(resp => {
            if (!utilMgr.isResponseOk(resp) && !utilMgr.isDeactivated()) {
                ...
                console.log(
                    "Code Time: Error sending data, saving kpm info offline"
                );
                sessionMgr.storePayload(payload);
...
export default {
    ...
    activate(state) {
        ...
        this.subscriptions.add(
            atom.commands.add("atom-workspace", {
                "Code-Time:code-time-metrics": () =>
                    sessionMgr.launchCodeTimeDashboard()
            })
        );
        this.subscriptions.add(
            atom.commands.add("atom-workspace", {
                "Code-Time:go-to-software.com": () => utilMgr.launchWebUrl()
            })
        );
        // this.subscriptions.add(
        //   atom.commands.add("atom-workspace", {
        //     "Code-Time:pause-code-time-metrics": () => sessionMgr.pauseMetrics()
        //   })
        // );
        // this.subscriptions.add(
        //   atom.commands.add("atom-workspace", {
        //     "Code-Time:enable-code-time-metrics": () => sessionMgr.enableMetrics()
        //   })
        // );
    ...
    activeTextEditorHandler() {
        ...
        if (
            atom.workspace.project &&
            atom.workspace.project.buffers &&
            atom.workspace.project.buffers.length > 0
        ) {
            let buffers = atom.workspace.project.buffers;
            for (let i = 0; i < buffers.length; i++) {
                let buffer = buffers[i];
                if (buffer && buffer.file && buffer.file.path) {
                    if (utilMgr.isCodeTimeMetricsFile(buffer.file.path)) {
        ...
        atom.workspace.observeTextEditors(editor => {
            ...
            if (fileName && utilMgr.isCodeTimeMetricsFile(fileName)) {
                utilMgr.updateDashboardIsOpen(true);
            }
            ...
            buffer.onDidDestroy(e => {
                if (fileName && utilMgr.isCodeTimeMetricsFile(fileName)) {
                    ...
                    utilMgr.updateDashboardIsOpen(false);
                }
