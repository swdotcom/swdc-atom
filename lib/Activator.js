...
function sendKeystrokeData() {
  ...
  Object.keys(keystrokeMgr.keystrokeCount.source).forEach(key => {
    ...
    const end = parseInt(keystrokeMgr.keystrokeCount.source[key]["end"], 10) || 0;
    if (end === 0) {
      ...
      keystrokeMgr.keystrokeCount.source[key]["local_end"] = nowTimes.local_now_in_sec;
    }
  });
  ...
  console.log(`Code Time: processing code time metrics: ${JSON.stringify(payload)}`);
...
export default {
  ...
  initializePlugin(state, initializedUser) {
    ...
    if (utilMgr.isCodeTime()) {
      ...
      this.subscriptions.add(
        atom.commands.add("atom-workspace", {
          "Music-Time:toggle-status-bar-metrics": () => utilMgr.toggleStatusBarMetrics()
        })
      );
      this.subscriptions.add(
        atom.commands.add("atom-workspace", {
          "Music-Time:launchTrackPlayer": () => musicControlMgr.launchTrackPlayer()
        })
      );

      this.subscriptions.add(
        atom.commands.add("atom-workspace", {
          "Music-Time:pause": () => musicControlMgr.pause(PlayerName.SpotifyWeb)
        })
      );
      this.subscriptions.add(
        atom.commands.add("atom-workspace", {
          "Music-Time:play": () => musicControlMgr.play(PlayerName.SpotifyWeb)
        })
      );
      this.subscriptions.add(
        atom.commands.add("atom-workspace", {
          "Music-Time:next": () => musicControlMgr.next(PlayerName.SpotifyWeb)
        })
      );
      this.subscriptions.add(
        atom.commands.add("atom-workspace", {
          "Music-Time:previous": () => musicControlMgr.previous(PlayerName.SpotifyWeb)
        })
      );
  ...
  switch (stat) {
    ...
    // if (editors.length < 1 ||
    //   (editors.length === 1 && !editors[0].getPath())) return utilMgr.alert('WARN', 'No file is opened!');
    ...
    console.log(rightDock.isVisible());
