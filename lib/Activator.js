  ...
  activeTextEditorHandler() {
    ...
    atom.workspace.onDidChangeActivePaneItem(editor => {
      if (
        ...
        if (utilMgr.isCodeTimeMetricsFile(editor.buffer.file.path)) {
          utilMgr.updateCodeTimeMetricsFileFocus(true);
          utilMgr.updateCodeTimeMetricsFileClosed(false);
        } else {
          utilMgr.updateCodeTimeMetricsFileFocus(false);
        }
      }
    });
    ...
    atom.workspace.observeTextEditors(editor => {
      ...
      if (utilMgr.isCodeTimeMetricsFile(fileName)) {
        utilMgr.updateCodeTimeMetricsFileFocus(true);
        utilMgr.updateCodeTimeMetricsFileClosed(false);
      } else {
        utilMgr.updateCodeTimeMetricsFileFocus(false);
      ...
      buffer.onDidDestroy(e => {
        if (utilMgr.isCodeTimeMetricsFile(fileName)) {
          utilMgr.updateCodeTimeMetricsFileFocus(false);
          utilMgr.updateCodeTimeMetricsFileClosed(true);
        } else {
          utilMgr.updateCodeTimeMetricsFileFocus(false);
        }
