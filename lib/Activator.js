...
export default {
    ...
    async initializePlugin(state, initializedUser) {
        ...
        this.toggleCodeTimeProvider('on');
    },
    ...
    toggleCodeTimeProvider(state) {
        if (!this.codetimeProvider)
            this.codetimeProvider = new CodeTimeProvider();
        const rightDock = atom.workspace.getRightDock();
        try {
            // Whatever do these first for performance
            rightDock.getPanes()[0].addItem(this.codetimeProvider);
            rightDock.getPanes()[0].activateItem(this.codetimeProvider);
        } catch (e) {
