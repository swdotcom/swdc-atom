...
export default {
    ...
    async activate(state) {
        ...
        if (
            utilMgr.isCodeTime() &&
            (!utilMgr.softwareSessionFileExists() || !utilMgr.jwtExists())
        ) {
    ...
    async initializePlugin(state, initializedUser) {
        ...
        if (utilMgr.isCodeTime()) {
            packageVersion = atom.packages.getLoadedPackage('code-time')
                .metadata.version
            ...
            packageVersion = atom.packages.getLoadedPackage('music-time')
                .metadata.version
            ...
            this.subscriptions.add(
                atom.commands.add('atom-workspace', {
                    'Music-Time:web-dashboard': () =>
                        musicMgr.displayMusicTimeMetricsMarkdownDashboard(),
                })
            )
            this.subscriptions.add(
                atom.commands.add('atom-workspace', {
