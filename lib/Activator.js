...
export default {
  ...
  initializePlugin(state, initializedUser) {
    ...
    if (!customDashMgr) {
      customDashMgr = new CustomDashboardManager(state);
    }
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        "Code-Time:custom-dashboard": () => this.toggleCustomDashboard()
      })
    )
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
  ...
  toggleCustomDashboard() {
    console.log('Custom Dashboard prompt was toggled!');
    return (
      customDashMgr.panel.isVisible() ?
      customDashMgr.panel.hide() :
      customDashMgr.panel.show()
    );
  },
