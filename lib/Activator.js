  ...
  activate(state) {
    ...
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        "Codetime:code-time-metrics": () => sessionMgr.launchCodeTimeDashboard()
      })
    );
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        "Codetime:go-to-software.com": () => sessionMgr.launchWebUrl()
      })
    );
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        "Codetime:pause-code-time-metrics": () => sessionMgr.pauseMetrics()
      })
    );
    this.subscriptions.add(
      atom.commands.add("atom-workspace", {
        "Codetime:enable-code-time-metrics": () => sessionMgr.enableMetrics()
      })
    );
    ...
    utilMgr
      ...
      .then(resp => {
        if (utilMgr.isResponseOk(resp)) {
          if (resp.data) {
            console.log("Code : Uninstalled plugin");
          } else {
            console.log(
              "Code : Failed to update Code  about the uninstall event"
            );
          }
        }
      });
      ...
      buffer.onDidStopChanging(e => {
        ...
        if (prevLines !== 0) {
          ...
          if (diff > 0) {
            ...
            console.log("Code : incremented lines added");
          } else if (diff < 0) {
            // lines were removed
            ...
            console.log("Code : incremented lines removed");
          }
        }
