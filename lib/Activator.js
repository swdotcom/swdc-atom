...
export default {
  ...
  async activate(state) {
    ...
    if (!utilMgr.softwareSessionFileExists()) {
  ...
  async initializeUserInfo() {
    ...
    if (user) {
      utilMgr.setItem("user", null);
    }
    let updateTime = utilMgr.getItem("atom_lastUpdateTime");
    if (updateTime) {
      utilMgr.setItem("atom_lastUpdateTime", null);
    }
