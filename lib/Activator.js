...
export default {
    ...
    async activate(state) {
        const serverIsOnline = await serviceUtil.serverIsAvailable();
        if (!utilMgr.softwareSessionFileExists() || !utilMgr.jwtExists()) {
            ...
            if (!serverIsOnline) {
                ...
                const result = await serviceUtil.createAnonymousUser(
                    serverIsOnline
                );
                if (!result) {
                    if (retry_counter === 0) {
    ...
    async initializePlugin(state, initializedUser, serverIsOnline) {
        ...
        payloadMgr.updateLastSavedKeystrokesStats();
    ...
    async initializeUserInfo(initializedUser, serverIsOnline) {
        ...
        if (!initializedAtomPlugin) {
            ...
            serviceUtil.sendHeartbeat('INSTALLED', serverIsOnline);
        }
