...
commandUtil.addCommands = subscriptions => {
    ...
    subscriptions.add(
        atom.commands.add('atom-workspace', {
            'Code-Time:web-dashboard': () => {
                // if not logged on, launch the onboarding url
                const isLoggedIn = userstatusMgr.getLoggedInCacheState();
                if (!isLoggedIn) {
                    userstatusMgr.launchLoginUrl();
                    eventMgr.createCodeTimeEvent(
                        'mouse',
                        'click',
                        'PaletteMenuLaunchOnboarding'
                    );
                } else {
    ...
    subscriptions.add(
        atom.commands.add('atom-workspace', {
            'Code-Time:update-session-summary': event => {
    ...
    subscriptions.add(
        atom.commands.add('atom-workspace', {
            'Code-Time:update-session-time': event => {
