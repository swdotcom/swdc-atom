...
let configSettingsTextUpdateCallback = null;
let configSettingsUpdateCallback = null;
...
export function showSettingsTextUpdatePrompt(title, prompt, callback, defaultInput = "") {
  configSettingsTextUpdateCallback = callback;
  showTextInputPrompt(title, prompt, "slackSettingsTextUpdateId", defaultInput);
}
...
export function showSettingsUpdateOptions(items, title, prompt, callback, defaultValueOption) {
  configSettingsUpdateCallback = callback;
...
$(document).on('change', '#settingsUpdateId', function() {
  const data = getSelectedData($(this).val());
  if (data) {
    configSettingsUpdateCallback(data.value);
  }
  closeListSelector();
});
...
$(document).on('click', '#slackSettingsTextUpdateId', function() {
  let data = $('#textInputElement').val();
  if (data) {
    configSettingsTextUpdateCallback(data);
  }
  closeInputSelector();
  return false;
});
