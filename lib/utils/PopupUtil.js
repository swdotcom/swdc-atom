...
export function showSlackMessageInputPrompt(callback) {
  slackMessageInputPromptCallback = callback;
  showTextInputPrompt("Slack message", "Enter a message to appear in your profile status", "slackMessageInputId");
}

export function showTextInputPrompt(title, textContent, listenerId) {
  // Create root element
  ...
  message.setAttribute("id", listenerId);
  inputPrompt.appendChild(message);
...
$(document).on('enter', '#slackMessageInputId', function() {
  console.log("message input val: ", $(this).val());
  closeListSelector();
});

$(document).on('click', '#listSelectorClose', function() {
    closeListSelector();
});
