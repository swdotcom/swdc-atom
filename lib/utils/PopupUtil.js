...
let configOptionsSettingsUpdateCallback = null;
...
function initiateAuthFlow(title, message, signup = false) {
  isSignup = signup
  const items = [
    {value: "", text: `${title} using...`},
...
export function showSettingsTextUpdatePrompt(title, prompt, callback, defaultInput = "") {
  slackMessageInputPromptCallback = callback;
  showTextInputPrompt(title, prompt, "slackMessageInputId", defaultInput);
}

// items should be in the following format:
// [{value, text},..]
export function showSettingsUpdateOptions(items, title, prompt, callback, defaultValueOption) {
  configOptionsSettingsUpdateCallback = callback;
  showListSelector(items, title, prompt, "settingsUpdateId", defaultValueOption);
}
...
export function showTextInputPrompt(title, textContent, listenerId, defaultInput) {
  ...
  okButton.setAttribute("id", "text-input-ok");
  okButton.innerHTML = '<button id="text-input-ok" class="prompt-input-button">Submit</button>';
...
export function showListSelector(items, title, message, listenerId, defaultValueOption = undefined) {
  ...
  listSelector.appendChild(messageDiv);
  listSelector.appendChild(selectListEl);

  if (defaultValueOption) {
    const defaultOption = document.createElement('option');
    defaultOption.value = defaultValueOption.value;
    defaultOption.text = defaultValueOption.text;
    selectListEl.appendChild(defaultOption);
  }
  items.forEach(item => {
    try {
      if (defaultValueOption) {
        if (defaultValueOption.value !== item.value) {
...
// settingsUpdateId: handle the selected setting option value
$(document).on('change', '#settingsUpdateId', function() {
  const data = getSelectedData($(this).val());
  if (data) {
    configOptionsSettingsUpdateCallback(data.value);
  }
  closeListSelector();
});
...
$(document).on('click', '#text-input-ok', function() {
...
$(document).on('change', '#authSelectId', function() {
  const data = getSelectedData($(this).val());
  if (data) {
    const switching_account = isSignup ? false : true;
    userstatusMgr.launchLoginUrl(data.value, switching_account);
  }
  closeListSelector();
});
