...
gitUtil.getChangeStats = async (projectDir, cmd) => {
     ...
     */
    const resultList = await execUtil.execCmd(cmd, projectDir, true);
...
gitUtil.getResourceInfo = async projectDir => {
    ...
    let branch = await execUtil.execCmd(
        'git symbolic-ref --short HEAD',
        projectDir
    );
    let identifier = await execUtil.execCmd(
        'git config --get remote.origin.url',
        projectDir
    );

    let email = await execUtil.execCmd(
        'git config user.email',
        projectDir
    );
    let tag = await execUtil.execCmd('git describe --all', projectDir);
...
gitUtil.getFileContributorCount = async (fileName) => {
    ...
    let resultList = await execUtil.execCmd(cmd, project_directory, true);
    if (!resultList) {
        // something went wrong, but don't try to parse a null or undefined str
        return 0;
    }
