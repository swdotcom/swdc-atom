...
gitUtil.getLastCommitId = async (projectDir, email) => {
    ...
    const list = await utilMgr.getCommandResultList(cmd, projectDir);
    if (list && list.length) {
...
gitUtil.getRepoConfigUserEmail = async projectDir => {
    ...
    return await utilMgr.getCommandResultLine(cmd, projectDir);
};

gitUtil.getRepoUrlLink = async projectDir => {
    const cmd = `git config --get remote.origin.url`;
    let str = await utilMgr.getCommandResultLine(cmd, projectDir);
