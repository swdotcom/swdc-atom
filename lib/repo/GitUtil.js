...
gitUtil.getFileContributorCount = async (fileName) => {
  ...
  if (!directory || !utilMgr.isGitProject(directory)) {
      return 0;
  }
  ...
  let resultList = await execUtil.getCommandResultList(cmd, directory);
  if (!resultList) {
      // something went wrong, but don't try to parse a null or undefined str
      return 0;
  }
...
gitUtil.getRepoContributorInfo = async (
    ...
    // get the repo url, branch, and tag
    let resourceInfo = await gitUtil.getResourceInfo(directory);
    if (resourceInfo && resourceInfo.identifier) {
