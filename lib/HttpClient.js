...
export async function spotifyApiPut(api, payload, accessToken) {
    ...
    return await spotifyApi.put(api, payload).catch(err => {
        utilMgr.logIt(`error posting data for ${api}, message: ${err.message}`);
        return err;
    });
}
...
export async function softwareGet(api, jwt) {
    ...
    return await beApi.get(api).catch(err => {
        utilMgr.logIt(`error fetching data for ${api}, message: ${err.message}`);
        return err;
    });
}
...
export async function isUserDeactivated(resp) {
    ...
    resp = await softwareGet("/users/ping", utilMgr.getItem("jwt"));
    if (isUnauthenticatedAndDeactivated(resp)) {
       utilMgr.showErrorStatus(
            "To see your coding data in Code Time, please reactivate your account."
        );
