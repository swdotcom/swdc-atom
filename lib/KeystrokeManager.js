...
export default class KeystrokeManager {
  ...
  constructor(projectName, projectDirectory, version) {
    this.keystrokeCount = new KeystrokeCount(projectName, projectDirectory, version);
  }
  ...
  updateFileInfoData(fileName, count, propertyToUpdate) {
    ...
    let fileInfo = this.findObjectInSource(fileName);
    if (!fileInfo) {
    ...
    fileInfo[fileName][propertyToUpdate] = fileInfo[fileName][propertyToUpdate] + count;

    if (propertyToUpdate === 'keys') {
  ...
  hasData() {
    if (this.keystrokeCount) {
        for (let i = 0; i < this.keystrokeCount.source.length; i++) {
            const fileInfo = this.keystrokeCount.source[i];
            if (fileInfo) {
   ...
   **/
  findObjectInSource(filenameToMatch) {
    for (let i = 0; i < this.keystrokeCount.source.length; i++) {
      const fileInfo = this.keystrokeCount.source[i];
