...
tracker.trackCodeTimeEvent = async (item) => {
  ...
  for (let i = 0; i < fileKeys.length; i++) {
    ...
    utilMgr.logIt(`Tracking Code Time Event: ${JSON.stringify(codetime_event, null, 2)}`);
...
tracker.trackEditorAction = async (entity, type, event = {}) => {
  ...
  utilMgr.logIt(`Tracking Editor Action Event: ${JSON.stringify(editor_event, null, 2)}`);
  // send the event
  swdcTracker.trackEditorAction(editor_event);
};

function isReady() {
  return jwtParams && jwtParams.jwt && trackerReady ? true : false;
}
...
async function getProjectParams() {
  ...
  console.log("dirNameInfo: ", dirNameInfo);
  return {
