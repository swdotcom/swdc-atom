...
import { sendOfflineTimeData, incrementSessionAndFileSecondsAndFetch } from "./TimeDataManager";
import { NO_PROJ_NAME, UNTITLED } from '../Constants';

const TimeCounterStats = require("../model/TimeCounterStats");
const FileChangeInfo = require("../model/FileChangeInfo");
const TimeData = require("../model/TimeData");
...
pluginDataMgr.processPayloadHandler = async (payload, sendNow, nowTimes, isUnfocus = false) => {
  ...
  payload.project = await projectMgr.getProjectInfo(payload.project);
  ...
  if (sendNow) {
    ...
    eventMgr.sendBatchPayload("/data/batch", [payload]);
    utilMgr.logIt(`sending kpm metrics`);
  } else {
...
async function midnightCheckHandler() {
  if (utilMgr.isNewDay()) {
    ...
    // clear the last saved keystroke stats
