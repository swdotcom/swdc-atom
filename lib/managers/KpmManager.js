...
kpmMgr.validateAndUpdateCumulativeData = async (payload, sessionMinutes) => {
    ...
    const td = await incrementSessionAndFileSecondsAndFetch(
        payload.project,
        sessionMinutes
    );
    ...
    let lastKpm = await payloadMgr.getLastSavedKeystrokeStats();
    if (lastKpm) {
        if (
        ...
        if (
            lastKpm &&
            timeUtil.getFormattedDay(lastKpm.start) !==
                timeUtil.getFormattedDay(payload.start)
        ) {
            // it's a new day
            lastKpm = null;
        }
