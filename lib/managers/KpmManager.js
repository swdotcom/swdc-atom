...
} from '../storage/SessionSummaryDataManager';
...
kpmMgr.sendKeystrokeData = async () => {
    ...
    const td: TimeData = await getTodayTimeDataSummary(data.project);
    ...
    Object.keys(data.source).forEach(key => {
        ...
        if (end === 0) {
            ...
            // set the cumulative editor seconds for each file
            data['cumulative_editor_seconds'] = editor_seconds;
        }
    });
    // update the time project summary
    incrementSessionAndFileSeconds(data.project);
