...
export function disconnectSlackAuth(authId) {
    ...
    if (!integration) {
        atom.notifications.addInfo('Disconnect Slack', {
            detail: 'Unable to find selected workspace to disconnect',
            dismissable: true,
        });
        return;
    }
    ...
    atom.confirm(
        ...
        async (resp) => {
            if (resp === 0) {
                await serviceUtil.softwareDelete(
...
export async function pauseSlackNotifications(
    ...
    initiateFlowRefresh = true,
    isFlowRequest = false
) {
    if (
        !isFlowRequest &&
        (!checkRegistration(true) || !checkSlackConnection(true))
    ) {
        return;
    }
    ...
    for (let i = 0; i < workspaces.length; i++) {
        ...
        const result = await web.dnd
            .setSnooze({ num_minutes: 120 })
            .catch((err) => {
    ...
    if (updated) {
        ...
        if (showSuccessNotification) {
            atom.notifications.addInfo('Pause notifications', {
                detail: 'Slack notifications are paused for 2 hours',
                dismissable: true,
            });
        }
...
export async function enableSlackNotifications(
    ...
    if (updated) {
        current_dnd_info = null;
        if (showSuccessNotification) {
            atom.notifications.addInfo('Enable notifications', {
                detail: 'Turned on Slack notifications',
                dismissable: true,
            });
        }
        if (initiateFlowRefresh) {
...
export async function getSlackDnDInfo() {
    ...
    for (let i = 0; i < workspaces.length; i++) {
        const workspace = workspaces[i];
        current_dnd_info = await getSlackDnDInfoPerDomain(
...
export async function getSlackStatus() {
    ...
    for (let i = 0; i < workspaces.length; i++) {
        ...
        current_slack_status =
            data && data.profile ? data.profile.status_text : '';
        break;
    }
    return current_slack_status;
}
...
export async function getSlackPresence() {
    ...
    for (let i = 0; i < workspaces.length; i++) {
        ...
        // set the cached var and return it
        current_slack_presence =
            data && data.presence ? data.presence : 'active';
        break;
    }
    return current_slack_presence;
}
...
export async function updateSlackPresence(
    presenceVal,
    showSuccessNotification = true,
    initiateFlowRefresh = true
) {
    ...
    if (updated) {
        ...
        if (showSuccessNotification) {
            atom.notifications.addInfo('Presence update', {
                detail: 'Slack presence updated',
                dismissable: true,
            });
        }
...
export function shareSlackMessageCallback(selected_channel, access_token) {
    ...
    showSlackMessageInputPrompt(
        shareSlackMessageCompletionCallback,
        shareMessage
    );
}
...
function getWorkspaceAccessToken(team_domain) {
    const integration = getSlackWorkspaces().find(
        (n) => n.team_domain === team_domain
    );
    if (integration) {
...
async function getChannelsCallback(selectedWorkspace) {
    ...
    delete web['axios'].defaults.headers['User-Agent'];
    const result = await web.conversations
...
async function refetchSlackConnectStatusLazily(tryCountUntilFoundUser) {
    ...
    if (!slackAuth) {
        ...
        atom.notifications.addInfo('Slack connect', {
            detail: 'Successfully connected to Slack',
            dismissable: true,
        });
...
async function getSlackAuth() {
    const { user } = await userstatusMgr.getUserRegistrationState(
        true /*isIntegration*/
    );
    ...
    if (user && user.registered === 1 && user.integrations) {
        ...
        for (const integration of user.integrations) {
            ...
            if (
                ...
                // check if it exists
                const foundIntegration = currentIntegrations.find(
...
function removeSlackIntegration(authId) {
    ...
    const newIntegrations = currentIntegrations.filter(
        (n) => n.authId !== authId
    );
...
async function getSlackDnDInfoPerDomain(team_domain) {
    ...
    if (accessToken) {
        ...
        dndInfo = await web.dnd.info().catch((e) => {
            console.error(
                'Error fetching slack do not disturb info: ',
                e.message
            );
            return null;
        });
