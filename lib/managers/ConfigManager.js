...
export function getConfigSettings() {
  return {
    ...
    flowModeReminders: atom.config.get("code-time.flowModeReminders"),
  }
}
...
export function editFlowModeReminders() {
  const configSettings = getConfigSettings();
  const title = "Flow Mode Reminders";
  const prompt = "Remind me to enable Flow Mode when I'm in flow";
  const items = [
    {value: true, text: "On"},
    {value: false, text: "Off"}
  ];
  const defaultValueText = configSettings.flowModeReminders ? "On" : "Off";
  const defaultValueOption = {value: configSettings.flowModeReminders, text: defaultValueText};
  showSettingsUpdateOptions(items, title, prompt, editFlowModeRemindersCallback, defaultValueOption);
}

export function editFlowModeRemindersCallback(value) {
  if(value !== undefined && value !== null) {
    atom.config.set("code-time.flowModeReminders", value);
    atom.commands.dispatch(
      atom.views.getView(atom.workspace),
      'Code-Time:refresh-flow-nodes'
    )
