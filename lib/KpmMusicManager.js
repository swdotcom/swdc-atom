 ...
 */
  ...
  async showSpotifyPlaylists(serverIsOnline) {
    ...
    if (this._spotifyPlaylists.length > 0) {
      ...
      filteredSpotifyPlaylists = filteredSpotifyPlaylists.map(element => {
        element['isPlaying'] = this._selectedPlaylistId == element.id ? true :false;
        return element
      });
      this.KpmMusicStoreManagerObj.selectedPlaylist = filteredSpotifyPlaylists[0] ? filteredSpotifyPlaylists[0] : null;
      this.structureViewObj.initialize(filteredSpotifyPlaylists,this._selectedPlaylistTrackId);
    }

    await getRunningTrack().then((track) => {
      
      if(track.state == 'paused') {
        $('#play-image-' + this._selectedPlaylistTrackId).attr("src",PLAY_CONTROL_ICON);

      }
      else if(track.state == 'playing') {
      
        $('#play-image-' + this._selectedPlaylistTrackId).attr("src",PAUSE_CONTROL_ICON);
  ...
  async showPlaylistItems(playlist_id) {
    ...
    if (playlist_id === SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
      ...
      this.KpmMusicStoreManagerObj.selectedPlaylist = this._playlistMap[playlist_id];
    ...
    if (playlistItem.type === "track") {
      ...
      if (playlistItem.playerType === PlayerType.MacItunesDesktop) {
        ...
        if (isLikedSongsPlaylist) {
          ...
          this.playSpotifySongInPlaylist(
  ...
  async playSpotifySongInPlaylist(
    playlist,
    track
  ) {
    const musicCtrlMgr = new MusicControlManager();
