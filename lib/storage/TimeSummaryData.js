...
timeSummaryData.getTimeDataSummaryFile = () => {
...
timeSummaryData.clearTimeDataSummary = () => {
    const data = new TimeData();
    timeSummaryData.saveTimeDataSummaryToDisk(data);
};
...
timeSummaryData.updateTimeSummaryData = (
    ...
    const timeData: TimeData = timeSummaryData.getTodayTimeDataSummary();
    ...
    timeSummaryData.saveTimeDataSummaryToDisk(timeData);
};

timeSummaryData.getTodayTimeDataSummary = () => {
    const nowTime = utilMgr.getNowTimes();
    const day = moment.unix(nowTime.local_now_in_sec).format('YYYY-MM-DD');

    let timeData = cacheMgr.get(`timeDataSummary_${day}`);
    if (!timeData) {
        const file = timeSummaryData.getTimeDataSummaryFile();
...
module.exports = timeSummaryData;
