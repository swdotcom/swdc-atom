 ...
 * @param spotifyUser
 */
export const launchAndPlayTrack = async (track, spotifyUser) => {
...
export const playSpotifySongInPlaylist = async (playlist, track) => {
    const musicCtrlMgr = new MusicControlManager()
...
export const playSelectedItem = async (playlistItem, isExpand = true) => {
    ...
    if (playlistItem.type === 'track') {
        ...
        if (!musicMgr.selectedPlaylist) {
            const playlist = await musicMgr.getPlaylistById(currentPlaylistId)
...
export class MusicPlaylistProvider {
    ...
    isTrackInPlaylistRunning(p) {
        return p.state === TrackStatus.Playing || p.state === TrackStatus.Paused
    }
    ...
    getTreeItem(p) {
        ...
        if (p.type === 'playlist') {
            ...
            if (p && p.tracks && p.tracks['total'] && p.tracks['total'] > 0) {
                const folderState = this.isTrackInPlaylistRunning(p)
