 ...
 */
  ...
  async refreshPlaylistForPlayer(playerName, serverIsOnline) {
    ...
    if (playerName === PlayerName.ItunesDesktop) {
      ...
      if (allowSpotifyPlaylistFetch) {
        ...
        this._playlistMap[SPOTIFY_LIKED_SONGS_PLAYLIST_ID] = this.getSpotifyLikedPlaylistFolder();
  ...
  async playSelectedItem(playlistItem, isExpand = true ) {
    ...
    let isLikedSongs = this._selectedPlaylistId === SPOTIFY_LIKED_SONGS_PLAYLIST_ID;
   ...
   */
  launchAndPlaySpotifyWebPlaylistTrack = async (isTrack: boolean) => {
