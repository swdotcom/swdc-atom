...
$(document).on("click", ".playlist-nested-item", async function(e) {
  e.stopPropagation();
  var _self = this;
  localStorage.setItem("_selectedPlaylistTrackId", $(_self).attr("node-id"));
  atom.commands.dispatch(
    atom.views.getView(atom.workspace),
...
export default class KpmMusicManager {
  ...
  async togglePLaylist() {
    ...
    let isCollapsed = $("[node-id=" + this._selectedPlaylistId + "]").hasClass(
      "collapsed"
    );
    if (isCollapsed) {
      ...
      $("[node-id=" + this._selectedPlaylistId + "]").removeClass("collapsed");
      ...
      $("[node-id=" + this._selectedPlaylistId + "]").addClass("collapsed");
      $("[node-id=" + this._selectedPlaylistId + "]").removeClass(
        "play-list-angle-down"
      );
      $("[node-id=" + this._selectedPlaylistId + "]").addClass(
  ...
  async playPlaylistSong() {
    ...
    this.selectedTrackItem = this._selectedPlaylist.filter(element => {
      return element.id == this._selectedPlaylistTrackId;
    })[0];
    await this.playSelectedItem(this.selectedTrackItem);
  ...
  async gatherMusicInfo() {
    ...
    const needsRefresh =
      ...
      Object.keys(this._playlistMap) == 0;
 ...
 */
  ...
  async showSpotifyPlaylists(serverIsOnline) {
    ...
    if (this._spotifyPlaylists.length > 0 && !this.requiresSpotifyAccess()) {
      let filteredSpotifyPlaylists = this._spotifyPlaylists.filter(element => {
        return element.type && element.type === "playlist";
      });
      let _self = this;
      filteredSpotifyPlaylists = filteredSpotifyPlaylists.map(async element => {
        element["isPlaying"] =
          this._selectedPlaylistId == element.id ? true : false;

        if (!element.child || element.child.length == 0) {
          element.child = await _self.getPlaylistItemTracksForPlaylistId(
            element.id
          );
        }
        return element;
      });
      const mappedelements = await Promise.all(filteredSpotifyPlaylists);

      // maps all songs on playlist
      //filteredSpotifyPlaylists = this.mapPLaylistItems(filteredSpotifyPlaylists);
      ...
      generatePLaylistButton["label"] =
        !customPlaylist || !this.isGenerateUsersWeeklyTopSongs
          ? GENERATE_CUSTOM_PLAYLIST_TITLE
          : REFRESH_CUSTOM_PLAYLIST_TITLE;
      generatePLaylistButton["tooltip"] == !customPlaylist ||
      !this.isGenerateUsersWeeklyTopSongs
        ? GENERATE_CUSTOM_PLAYLIST_TOOLTIP
        : REFRESH_CUSTOM_PLAYLIST_TOOLTIP;
    ...
    await getRunningTrack().then(track => {
      if (track.state == "paused") {
        $("[node-id=" + this._selectedPlaylistId + "]").find("#play-image-" + this._selectedPlaylistTrackId).attr(
          "src",
          PLAY_CONTROL_ICON
        );
    ...
    if (playerName === PlayerName.ItunesDesktop) {
      ...
      if (this._musictimePlaylists && this._musictimePlaylists.length > 0) {
        for (let i = 0; i < this._musictimePlaylists.length; i++) {
          ...
          if (musicTimePlaylist.playlistTypeId === PERSONAL_TOP_SONGS_PLID) {
            items.push(musicTimePlaylist);
          }
        }
      }
    ...
    if (spotifyPlaylists && spotifyPlaylists.length > 0) {
      spotifyPlaylists = spotifyPlaylists.map(async function(item) {
        ...
        if (!item.child || item.child.length == 0) {
          item.child = await _self.getPlaylistItemTracksForPlaylistId(item.id);
        }
      });
    }
  ...
  async showPlaylistItems(playlist_id) {
    if (playlist_id === SPOTIFY_LIKED_SONGS_PLAYLIST_ID) {
      ...
      this._selectedPlaylist = this.structureViewObj._spotifyPlaylists ?  this.structureViewObj._spotifyPlaylists.filter( element => {
        return element.id == playlist_id;
      })[0].child : [];
    }
  ...
  async getActiveSpotifyDevicesTitleAndTooltip() {
    ...
    return {
      ...
      tooltip: "No Spotify devices found, you may need to login to your player",
      loggedIn: false
    };
  }
  ...
  async generateUsersWeeklyTopSongs() {
    this.isGenerateUsersWeeklyTopSongs = true;
