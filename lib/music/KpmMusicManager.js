 ...
 */
  ...
  async updateSavedPlaylists(playlist_id, playlistTypeId, name) {
    ...
    let createResult = await softwarePost("/music/generatedPlaylist", payload, jwt);
  ...
  async playSelectedItem(playlistItem, isExpand = true) {
    ...
    if (playlistItem.type === "track") {
      let currentPlaylistId = playlistItem["playlist_id"] ? playlistItem["playlist_id"] : playlistItem["id"];
