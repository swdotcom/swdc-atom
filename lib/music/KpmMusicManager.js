...
export default class KpmMusicManager {
  ...
  async togglePLaylist() {
    ...
    let isCollapsed = $("[node-id=" + this._selectedPlaylistId + "]").hasClass("collapsed");
    if (isCollapsed) {
      await this.refreshPlaylists();
      await this.showPlaylistItems(this._selectedPlaylistId);
      $("[node-id=" + this._selectedPlaylistId + "]").removeClass("collapsed");
      $("[node-id=" + this._selectedPlaylistId + "]").addClass("play-list-angle-down");
      $("[node-id=" + this._selectedPlaylistId + "]").removeClass("play-list-angle-right");
    } else {
      $("[node-id=" + this._selectedPlaylistId + "]").addClass("collapsed");
      $("[node-id=" + this._selectedPlaylistId + "]").removeClass("play-list-angle-down");
      $("[node-id=" + this._selectedPlaylistId + "]").addClass("play-list-angle-right");
    }
    return;
  }

  async playPlaylistSong() {
    let playlistId = localStorage.getItem('_selectedPlaylistTrackId');
  ...
  async gatherMusicInfo() {
    ...
    const needsRefresh =
      changeStatus.isNewTrack || changeStatus.trackStateChanged || Object.keys(this._playlistMap) == 0;
