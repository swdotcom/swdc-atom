 ...
 */
    ...
    async showSpotifyPlaylists(serverIsOnline) {
        ...
        if (
            ...
            // )
            let _self = this
            let filteredSpotifyPlaylists = this._spotifyPlaylists.map(
                element => {
                    element['isPlaying'] =
            ...
            generatePLaylistButton['label'] = !this.globalPlaylistIdExists()
                ? GENERATE_CUSTOM_PLAYLIST_TITLE
                : REFRESH_CUSTOM_PLAYLIST_TITLE
            generatePLaylistButton['tooltip'] == !this.globalPlaylistIdExists()
                ? GENERATE_CUSTOM_PLAYLIST_TOOLTIP
                : REFRESH_CUSTOM_PLAYLIST_TOOLTIP
    ...
    async initializeSpotify() {
        ...
        if (
            ...
            if (oauthResult && oauthResult.auth) {
                // update the CodyMusic credentials
                await this.KpmMusicStoreManagerObj.updateSpotifyAccessInfo(
                    oauthResult.auth
                )
            } else {
                utilMgr.setItem('spotify_access_token', null)
