...
export default class KpmMusicTimeStatusView {
  ...
  async display(msg, icon,tooltip, track) {
    ...
    let needsSpotifyAccess = this.musicstoreMgr ? this.musicstoreMgr.requiresSpotifyAccess() : true;

    // there's nothing to get if it's windows and they don't have
    // a premium spotify account  
    let premiumAccountRequired = this.musicstoreMgr ?
      !utilMgr.isMac() && !this.musicstoreMgr.hasSpotifyPlaybackAccess() ? true : false : true;
    ...
    await getRunningTrack().then((track) => {
      ...
      if(track.state !== 'notassigned' && !needsSpotifyAccess && !premiumAccountRequired) {
