...
export default class SessionManager {
  ...
  async isUserAuthenticated() {
    ...
    const tokenVal = appConstants.getItem("token");
    if (!tokenVal) {
      return false;
    }
  ...
  storePayload(payload) {
    fs.appendFile(
      appConstants.getSoftwareDataStoreFile(),
  ...
  sendOfflineData() {
    ...
    const dataStoreFile = appConstants.getSoftwareDataStoreFile();
      ...
      if (content) {
        ...
        return appConstants
          ...
          .then(response => {
            ...
            this.deleteFile(appConstants.getSoftwareDataStoreFile());
          })
          .catch(err => {
            console.log(
  ...
  checkUserAuthenticationStatus() {
    ...
    const tokenVal = appConstants.getItem("token");
    const lastUpdateTime = appConstants.getItem("atom_lastUpdateTime");
    ...
    Promise.all([serverAvailablePromise, isAuthenticatedPromise]).then(
      values => {
        ...
        if (
          ...
          appConstants.setItem("atom_lastUpdateTime", Date.now());
          let infoMsg =
   ...
   */
  isPastTimeThreshold() {
    const existingJwt = appConstants.getItem("jwt");
    const thresholdHoursBeforeCheckingAgain = appConstants.getShortThresholdHours();
    const lastUpdateTime = appConstants.getItem("atom_lastUpdateTime");
  ...
  checkTokenAvailability() {
    ...
    appConstants
      ...
      .then(response => {
        if (response.data) {
          appConstants.setItem("jwt", response.data.jwt);
          appConstants.setItem("user", response.data.user);
          appConstants.setItem("atom_lastUpdateTime", Date.now());
        }
      })
      .catch(err => {
  ...
  async pauseMetrics() {
    ...
    appConstants
      ...
      .display(
        "Paused",
        await appConstants.getLaunchUrl(),
        "",
  ...
  async showStatus(msg, icon) {
    appConstants
      .getStatusView()
      .display(msg, await appConstants.getLaunchUrl(), icon);
  }
}
