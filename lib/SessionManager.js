...
sessionMgr.checkUserAuthenticationStatus = async () => {
  ...
  if (
    serverAvailable &&
    !isAuthenticated &&
    (pastThresholdTime || !lastUpdateTime)
  ) {
...
sessionMgr.launchCodeTimeDashboard = async () => {
  let alreadyFocused = utilMgr.isCodeTimeMetricsFocused();
  let isClosed = utilMgr.isCodeTimeMetricsClosed;
  ...
  let shouldFocus = alreadyFocused || isClosed ? true : false;
  ...
  let legacyFile = file.substring(0, file.lastIndexOf("."));
  ...
  utilMgr
    ...
    .then(resp => {
      if (utilMgr.isResponseOk(resp)) {
        ...
        let alreadyFocused = utilMgr.isCodeTimeMetricsFocused();
        let alreadyClosed = utilMgr.isCodeTimeMetricsClosed();
        if (alreadyFocused && !alreadyClosed) {
