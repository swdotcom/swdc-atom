...
utilMgr.syncSlackIntegrations = (integrations) => {
  const nonSlackIntegrations = utilMgr.getIntegrations().filter(
    (integration: any) => !utilMgr.isActiveIntegration('slack', integration)
  );
  integrations = integrations && integrations.length ? [...integrations, ...nonSlackIntegrations] : nonSlackIntegrations;
  fileIt.writeJsonFileSync(utilMgr.getIntegrationsFile(), integrations);
}
