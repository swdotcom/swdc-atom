...
utilMgr.getIntegrations = () => {
  let integrations = fileIt.getJsonValue(utilMgr.getIntegrationsFile());
  if (!integrations) {
    integrations = [];
...
utilMgr.syncIntegrations = (integrations) => {
  fileIt.setJsonValue(utilMgr.getIntegrationsFile(), integrations);
}
