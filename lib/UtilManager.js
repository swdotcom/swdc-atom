...
utilMgr.getAuthenticatedPluginAccounts = async (identityId, token = null) => {
  ...
  if (authenticatingJwt && serverIsOnline && identityId) {
...
utilMgr.getAnonymousUser = (identityId, authAccounts) => {
  if (authAccounts && authAccounts.length > 0) {
    for (let i = 0; i < authAccounts.length; i++) {
      let user = authAccounts[i];
      let userMacAddr = user.mac_addr;
...
utilMgr.getUserStatus = async (token = null) => {
  ...
  let anonUser = utilMgr.getAnonymousUser(identityId, authAccounts);
  if (anonUser) {
    ...
    anonUser = utilMgr.getAnonymousUser(identityId, authAccounts);
  }
