...
utilMgr.getProjectContributorCodeSummaryFile = () => {
    let file = utilMgr.getSoftwareDir();
    if (utilMgr.isWindows()) {
...
utilMgr.getMinutesSinceLastPayload = () => {
    ...
    if (lastPayloadEnd) {
        ...
        if (
            diffInSec > 0 &&
            diffInSec <= utilMgr.getSessionThresholdSeconds()
        ) {
...
utilMgr.generateContributorSummary = projectDir => {
    const file = utilMgr.getProjectContributorCodeSummaryFile();
    ...
    // if (fs.existsSync(file)) {
    //     const summaryContent = fs.readFileSync(file).toString();
    //
    // }
