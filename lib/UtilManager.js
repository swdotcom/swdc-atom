...
utilMgr.getCommandResult = async (cmd) => {
    let result = "";
    let content = "";
    if (utilMgr.isWindows()) {
        content = await utilMgr.wrapExecPromise(`cmd /c ${cmd}`, null);
    } else {
        // use the windows commmand
        content = await utilMgr.wrapExecPromise(`/bin/sh -c ${cmd}`, null);
    }
  ...
  if (appJwt) {
    ...
    if (!jwt && !cachedJwt) {
      ...
      let resp = await utilMgr.softwarePost(`/data/onboard`,
        { timezone, username, creation_annotation },
        appJwt
      );
      if (
        utilMgr.isResponseOk(resp) &&
...
utilMgr.getUserStatus = async (token = null) => {
  ...
  if (serverIsOnline) {
      ...
      if (!loggedInResp.loggedOn && cachedJwt && jwt !== cachedJwt) {
          ...
          loggedInResp = await utilMgr.isLoggedOn(serverIsOnline, cachedJwt);
      }
