...
utilMgr.launchCodeTimeDashboard = async (options = {}) => {

  let file = null;

  if (options.start) {
    file = utilMgr.getCustomDashboardFile();
  } else {
    file = utilMgr.getDashboardFile();
  }

  const queryString = utilMgr.buildQueryString(options);
...
utilMgr.buildQueryString = options => {
