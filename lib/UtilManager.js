...
utilMgr.getAuthenticatedPluginAccounts = async (macAddr, token = null) => {
  ...
  if (authenticatingJwt && serverIsOnline && macAddr) {
...
utilMgr.getLoggedInUser = (macAddr, authAccounts) => {
...
utilMgr.hasRegisteredUserAccount = (macAddr, authAccounts) => {
  if (authAccounts && authAccounts.length > 0) {
    for (let i = 0; i < authAccounts.length; i++) {
      let user = authAccounts[i];
      let userMacAddr = user.mac_addr;
...
utilMgr.getAnonymousUser = (macAddr, authAccounts) => {
  if (authAccounts && authAccounts.length > 0) {
    for (let i = 0; i < authAccounts.length; i++) {
      let user = authAccounts[i];
      let userMacAddr = user.mac_addr;
...
utilMgr.getUserStatus = async (token = null) => {
  ...
  let loggedInUser = utilMgr.getLoggedInUser(macAddress, authAccounts);
  let anonUser = utilMgr.getAnonymousUser(macAddress, authAccounts);
  if (anonUser) {
    ...
    anonUser = utilMgr.getAnonymousUser(macAddress, authAccounts);
  }
