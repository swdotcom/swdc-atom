...
utilMgr.displayReadmeIfNotExists = (override = false) => {
    ...
    //         ViewColumn.One
    //     );
    //     utilMgr.setItem("vscode_CtReadme", true);
    // }
};
...
utilMgr.updateStatusBarWithSummaryData = (data, wcTime) => {
    if (wcTime === null || wcTime === undefined) {
        utilMgr.showStatus('Code Time', null);
    }

    let currentDayMinutes = data ? data.currentDayMinutes : 0;
    let averageDailyMinutes = data ? data.averageDailyMinutes : 0;
...
utilMgr.launchCodeTimeDashboard = async () => {
    ...
    if (!fs.existsSync(file)) {
        await dashboardMgr.getSessionSummaryStatus();
