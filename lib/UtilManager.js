...
utilMgr.getUserStatus = async (token = null) => {
  ...
  utilMgr.updateMenuPreference(LOGOUT_COMMAND_KEY, loggedIn);
...
utilMgr.updateMenuPreference = (command, flag) => {
  ...
  // this is sent from the getUserStatus call
  if (command === LOGOUT_COMMAND_KEY) {
    // the flag is "isLoggedOn" set to True or False
    if (flag) {
      utilMgr.removeMenuItem(LOGIN_MENU_LABEL);
      alreadyFound = utilMgr.addMenuItem(WEB_DASHBOARD_MENU_LABEL, WEB_DASHBOARD_COMMAND_KEY);
    } else {
      alreadyFound = utilMgr.addMenuItem(LOGIN_MENU_LABEL, LOGIN_COMMAND_KEY);
      utilMgr.removeMenuItem(WEB_DASHBOARD_MENU_LABEL);
    }
    if (!alreadyFound) {
      updatePref = true;
    }
  } else if (command === SHOW_MUSIC_METRICS_CONFIG_KEY) {
    let prefLabel = TOP_MUSIC_MENU_LABEL;
    if (!flag) {
      utilMgr.removeMenuItem(prefLabel);
    } else {
      alreadyFound = utilMgr.addMenuItem(prefLabel, command);
    }
  }
