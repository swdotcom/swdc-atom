...
export default class DownloadManager {
  ...
  getPluginManagerInstallDir() {
    if (this.isMac()) {
      ...
      return os.homedir() + "\\AppData\\Local\\Programs";
    } else {
      return "/usr/lib/";
    }
  }
  ...
  downloadPM() {
    ...
    req.on("response", function(data) {
      if (data && data.statusCode === 200) {
        console.log("Downloading Software plugin manager...");
      } else {
        console.log(
          "Unable to download Software plugin manager, request status: ",
          data.statusCode
        );
    ...
    req.on("data", function(chunk) {
      ...
      const percent = Math.ceil(
        Math.max(received_bytes * 100 / total_bytes, 2)
      );
      if (percent !== previousPercent && percent % 10 === 0) {
        console.log(`Downloading Software plugin manager: ${percent}%`);
      }
